...
-- CREATED BY/DATE     : Rapid SQL on 2/26/2005 02:36:33.776
 ...
 *************************************************************************
 * $Id: DatabaseBuild.sql,v 1.1.1.5 2005/03/08 11:44:25 marco Exp $
 ***
...
--	DBMS_OUTPUT.PUT_LINE('BOM');
	FOR bom IN CUR_BOM LOOP
		...
		IF (bom.ProductType = 'I' AND bom.ProductType = 'I') THEN
			--	Get ProductQty
			SELECT 	NVL(SUM(QtyOnHand), 0)
			  INTO	ProductQty
			FROM 	M_Storage s
        ...
        FOR s IN Cur_PaySchedule LOOP
            IF (s.C_InvoicePaySchedule_ID = p_C_InvoicePaySchedule_ID) THEN
                ...
                IF (s.DueAmt - v_Remaining < 0) THEN
                    v_TotalOpenAmt := 0;
                END IF;
--              DBMS_OUTPUT.PUT_LINE('Sched Total=' || v_TotalOpenAmt || ', Due=' || s.DueAmt || ',CM=' || v_MultiplierCM);
...
CREATE OR REPLACE PROCEDURE C_AcctSchema_Default_Copy
(
...
CREATE OR REPLACE PROCEDURE C_BP_Group_Acct_Copy
(
	PInstance_ID			IN NUMBER,
 ...
 *************************************************************************
 * $Id: DatabaseBuild.sql,v 1.1.1.5 2005/03/08 11:44:25 marco Exp $
 * $Source: /opt/mercurial/repository/CVSRepo/compiere/compiere-all/db/database/DatabaseBuild.sql,v $
 ***
...
-- File ad_element_trl_trg.sql was not found.

-- File ad_menu_trg.sql was not found.

-- File ad_process_trg.sql was not found.

-- File ad_process_trl_trg.sql was not found.

-- File ad_table_trg.sql was not found.

-- File ad_tree_trg.sql was not found.

-- File ad_window_trg.sql was not found.

-- File ad_window_trl_trg.sql was not found.

-- File ad_workflow_trg.sql was not found.

-- File c_acctschema_element_trg.sql was not found.

-- File c_bankaccount_trg.sql was not found.

-- File c_bp_group_trg.sql was not found.

-- File c_bpartner_trg.sql was not found.

-- File c_cashbook_trg.sql was not found.

-- File c_charge_trg.sql was not found.

-- File c_commissionamt_trg.sql was not found.

-- File c_commissiondetail_trg.sql was not found.

-- File c_elementvalue_trg.sql was not found.

-- File c_period_trg.sql was not found.

-- File c_period_trg2.sql was not found.

-- File c_project_trg.sql was not found.

-- File c_project_trg2.sql was not found.

-- File c_projectline_trg.sql was not found.

-- File c_projectline_trg2.sql was not found.

-- File c_revenuerecognition_plan_trg.sql was not found.

-- File c_tax_trg.sql was not found.

-- File c_withholding_trg.sql was not found.

-- File m_product_bom_trg.sql was not found.

-- File m_product_category_trg.sql was not found.

-- File m_product_trg.sql was not found.

-- File m_warehouse_trg.sql was not found.
...
CREATE OR REPLACE PROCEDURE AD_Tab_Import
(
	p_PInstance_ID    		IN NUMBER,
	p_AD_Tab_ID				IN NUMBER 	--	DEFAULT NULL
)
/*************************************************************************
 * The contents of this file are subject to the Compiere License.  You may
 * obtain a copy of the License at    http://www.compiere.org/license.html 
...
BEGIN
	...
	WHERE	AD_Tab_ID = v_Record_ID
	  AND	AD_Table_ID != 291;	-- C_BPartner (multiple tabs)

	FOR CC IN Cur_Column (v_Record_ID, v_AD_Table_ID) LOOP
		...
		VALUES
			...
			0, v_Record_ID, CC.AD_Column_ID, CC.FieldLength, 
			'Y', CC.EntityType);
		--
		v_count := v_count + 1;
		IF (p_PInstance_ID IS NOT NULL) THEN
			INSERT INTO AD_PInstance_Log (AD_PInstance_ID, Log_ID, 
			  ...
			  VALUES (p_PInstance_ID, AD_PInstance_Seq.NextVal, 
			  	'@Created@ @AD_Field_ID@ ' || CC.Name);
		ELSE
			DBMS_OUTPUT.PUT_LINE(' adding ' || CC.Name);
		END IF;
...
END AD_Tab_Import;
/
CREATE OR REPLACE PROCEDURE AD_Table_Import
(
	p_PInstance_ID    		IN NUMBER,
	p_AD_Table_ID			IN NUMBER	--	DEFAULT NULL
)
/*************************************************************************
 * The contents of this file are subject to the Compiere License.  You may
 * obtain a copy of the License at    http://www.compiere.org/license.html 
	...
	--	
   	CURSOR Cur_Column (tableID NUMBER)	IS
        ...
        WHERE   uc.Table_Name=UPPER(t.TableName)
			...
			AND (tableID=t.AD_Table_ID OR tableID IS NULL);
   	--
	v_NextNo		NUMBER(10);
	v_count			NUMBER(10) := 0;
...
BEGIN
	...
	FOR CC IN Cur_Column (v_Record_ID) LOOP
		...
		VALUES
			...
			CC.AD_Table_ID, 
			DECODE(CC.Data_Type, 'NUMBER', 11,	--	Integer 
				'CHAR', 20, 					--	YesNo
				'DATE', 15, 					--	Date
				10),							--	String
		...
		IF (p_PInstance_ID IS NOT NULL) THEN
			INSERT INTO AD_PInstance_Log (AD_PInstance_ID, Log_ID, 
				P_Msg)
			  VALUES (p_PInstance_ID, AD_PInstance_Seq.NextVal, 
			  	'@Created@ @AD_Column_ID@ ' || InitCap(CC.Table_Name) || '.' || InitCap(CC.Column_Name));
		ELSE
            DBMS_OUTPUT.PUT_LINE('adding Table ' || InitCap(CC.Table_Name) || ' Column ' || InitCap(CC.Column_Name));
		END IF;
...
-- File ad_pinstance_trg.sql was not found.
...
CREATE OR REPLACE PROCEDURE AD_Tab_Import0
(
	p_PInstance_ID    		IN NUMBER
)
/*************************************************************************
...
END AD_Tab_Import0;
/
CREATE OR REPLACE PROCEDURE AD_Table_Import0
(
	p_PInstance_ID    		IN NUMBER
)
/*************************************************************************
...
-- File s_resourcetype_trg.sql was not found.

-- File s_resourceunavailable_trg.sql was not found.

-- File AD_PrintFormatItem_BTrg.sql was not found.

-- File AD_PrintFormatItem_Trg.sql was not found.
...
-- File s_resource_trg.sql was not found.

-- File s_expensetype_trg.sql was not found.
...
-- File S_ResourceAssignment_Trg.sql was not found.
...
SELECT trl.AD_Language, t.AD_Tab_ID, t.AD_Window_ID, t.AD_Table_ID, trl.Name, trl.Description, 
    ...
    tbl.IsHighVolume, tbl.IsView, 'N' AS HasAssociation, -- compatibility
    t.IsTranslationTab, t.IsReadOnly, t.AD_Image_ID, t.TabLevel, 
    t.WhereClause, t.OrderByClause, trl.CommitWarning, 
    t.AD_Column_ID, t.AD_Process_ID, t.IsSortTab, 
    t.AD_ColumnSortOrder_ID, t.AD_ColumnSortYesNo_ID, t.Included_Tab_ID
FROM AD_Tab t 
	INNER JOIN AD_Table tbl ON (t.AD_Table_ID = tbl.AD_Table_ID)
...
SELECT il.AD_Client_ID, il.AD_Org_ID, il.IsActive, il.Created, il.CreatedBy, il.Updated, il.UpdatedBy,
	...
	COALESCE(p.Name||productAttribute(il.M_AttributeSetInstance_ID), il.Description) AS Name, -- main line
...
SELECT il.AD_Client_ID, il.AD_Org_ID, il.IsActive, il.Created, il.CreatedBy, il.Updated, il.UpdatedBy,
	...
	CASE WHEN COALESCE(pt.Name,p.Name)||productAttribute(il.M_AttributeSetInstance_ID) IS NOT 
        NULL THEN il.Description END AS Description, -- second line
...
SELECT ol.AD_Client_ID, ol.AD_Org_ID, ol.IsActive, ol.Created, ol.CreatedBy, ol.Updated, ol.UpdatedBy,
	...
	COALESCE(p.Name||productAttribute(ol.M_AttributeSetInstance_ID), ol.Description) AS Name, -- main line
...
SELECT ol.AD_Client_ID, ol.AD_Org_ID, ol.IsActive, ol.Created, ol.CreatedBy, ol.Updated, ol.UpdatedBy,
    ...
    CASE WHEN ol.QtyEntered<>0 OR ol.M_Product_ID IS NOT NULL THEN ol.QtyEntered END AS QtyEntered, 
    CASE WHEN ol.QtyEntered<>0 OR ol.M_Product_ID IS NOT NULL THEN uom.UOMSymbol END AS UOMSymbol, 
	COALESCE(COALESCE(pt.Name, p.Name)||productAttribute(ol.M_AttributeSetInstance_ID), ol.Description) AS Name, -- main line
...
SELECT iol.AD_Client_ID, iol.AD_Org_ID, iol.IsActive, iol.Created, iol.CreatedBy, iol.Updated, iol.UpdatedBy,
	...
	COALESCE(p.Name||productAttribute(iol.M_AttributeSetInstance_ID), iol.Description) AS Name, -- main line
...
SELECT iol.AD_Client_ID, iol.AD_Org_ID, iol.IsActive, iol.Created, iol.CreatedBy, iol.Updated, iol.UpdatedBy,
    ...
    ol.QtyOrdered, ol.QtyDelivered,
	CASE WHEN iol.MovementQty<>0 OR iol.M_Product_ID IS NOT NULL THEN ol.QtyOrdered-ol.QtyDelivered END AS QtyBackOrdered,
	COALESCE(COALESCE(pt.Name,p.Name)||productAttribute(iol.M_AttributeSetInstance_ID), iol.Description) AS Name, -- main line
...
SELECT R_Request_ID, AD_Client_ID,AD_Org_ID, IsActive, Created,CreatedBy, Updated, UpdatedBy,
    ...
    R_RequestType_ID, SalesRep_ID, IsSelfService, DateLastAlert,
    AD_User_ID, C_BPartner_ID, C_Campaign_ID, C_Order_ID, C_Invoice_ID, C_Payment_ID,
    M_Product_ID, C_Project_ID, A_Asset_ID, AD_Table_ID, Record_ID,
    ActionType, R_MailText_ID, MailSubject, MailText, Result, NextAction, DateNextAction,
...
SELECT i.AD_Org_ID, i.AD_Client_ID, 
	...
	i.DateInvoiced, 
    p.NetDays, 
	addDays(i.DateInvoiced,p.NetDays) AS DueDate,
...
WHERE	(o.DocStatus = 'CO' AND o.IsDelivered='N')
	...
	AND o.IsDropShip='N' AND l.M_Product_ID IS NOT NULL
